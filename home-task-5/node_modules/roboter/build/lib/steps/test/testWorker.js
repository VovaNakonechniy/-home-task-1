...
import { loadGlobalPostScript, loadGlobalPreScript, loadTestTypePostScript, loadTestTypePreScript } from './loadTestScript';
import Mocha from 'mocha';
import { parentPort, workerData } from 'worker_threads';
const { applicationRoot, absoluteTestFilesPerType, typeSequence, bail, grep, watch, previousRunResult, runNumber } = workerData;
const globalPreScript = await loadGlobalPreScript({ applicationRoot });
let globalPreScriptResult;
if (globalPreScript) {
    buntstift.line();
...
for (const testType of typeSequence) {
    ...
    const absoluteTestTypeDirectory = path.join(applicationRoot, 'test', testType);
