...
suite('build', function () {
    ...
    testWithFixture('builds sources and tests using standard config.', ['with-valid-typescript'], async (fixture) => {
    ...
    testWithFixture('builds using the packaged typescript version.', ['build', 'with-typescript-feature-that-requires-newer-version'], async (fixture) => {
        const roboterResult = await runCommand('npx roboter build', {
            cwd: fixture.absoluteTestDirectory,
            silent: true
        });
    ...
    testWithFixture('fails-when-build-fails.', ['build', 'with-invalid-typescript'], async (fixture) => {
        const roboterResult = await runCommand('npx roboter build', {
            cwd: fixture.absoluteTestDirectory,
            silent: true
        });
    ...
    testWithFixture('runs-pre-and-post-tasks.', ['build', 'with-prebuild-and-postbuild-task'], async (fixture) => {
        const roboterResult = await runCommand('npx roboter build', {
            cwd: fixture.absoluteTestDirectory,
            silent: true
        });
    ...
    testWithFixture('skips compilation without typescript.', ['with-valid-javascript'], async (fixture) => {
        const roboterResult = await runCommand('npx roboter build', {
            cwd: fixture.absoluteTestDirectory,
            silent: true
        });
    ...
    testWithFixture('supports incremental flag.', ['build', 'with-tsconfig-incremental-flag-true'], async (fixture) => {
        const roboterResult = await runCommand('npx roboter build', {
            cwd: fixture.absoluteTestDirectory,
            silent: true
        });
        assert.that(roboterResult).is.aValue();
        const builtFile = await fs.promises.readFile(path.join(fixture.absoluteTestDirectory, 'build', 'index.js'), { encoding: 'utf8' });
